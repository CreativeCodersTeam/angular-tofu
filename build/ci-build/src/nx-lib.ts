...
export class NxLib {
  ...
  async getAffectedProjects(): Promise<string[]> {
    const baseArg = this.base ? `--base=${this.base}` : '';
    const affected = await this.executeNxCommand(`print-affected ${baseArg} --select=projects`);
    return affected.split(',').map(project => project.trim()).filter(project => project.length > 0);
  }
